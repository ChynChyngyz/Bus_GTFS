...
class Model:
    ...
    class Workspace:
        ...
        def create(self, lat, lon, r):
            self.model.controller.showLoading("Создание базы данных")
            ...
            def insertCSV(tableName, filter = lambda header, row : True):
                self.model.controller.showLoading("Импорт данных : %s" % tableName)
                nInserted = 0
                with open('%s%s.txt' % (self.path, tableName), 'r', encoding = 'utf8') as f:
                    csvReader = csv.reader(f)
                    header = next(csvReader)
            ...
            self.model.controller.showLoading("Ассоциация остановок и дорог")
            cursor = db.cursor()
            ...
            self.model.controller.showLoading("Заполнение расписания")
            cursor = db.cursor()
                ...
                if memo.is_ready():
                    ...
                    if stoptime_1[0] == stoptime_2[0]:
                        ...
                        if i >= n:
                            i = 0
                            k += 1
                            print('Обработанный %s' % (k * n))

                        i += 1
            db.commit()

            self.model.controller.showLoading("Добавление пересадок в расписание")
            ...
            self.model.controller.showLoading("Составление списка преемников")
            cursor = db.cursor()
            ...
            memo = Memo(2)
            localStorage = LmdbDataStore('%s/успех' % self.path, dict())
            successors = {}
    ...
    def dijkstra(self, from_stop_id, departure_time_string, to_stop_id, modes):
        ...
        try:
            ...
            return self.controller.showError("Два узла не соединены")
